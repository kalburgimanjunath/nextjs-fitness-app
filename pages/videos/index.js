  ...
  const VideoCard = ({ item }) => {
    return (
      <div className="video-item">
        <div className="video-thumb" onClick={() => OpenModal(item)}>
          <img src={item.thumb} width="100%" height="200" />
        </div>
        <div className="video-name">
          <div className="video-title">{item.title}</div>
          <div className="video-subtitle">{item.subtitle}</div>
        </div>
  ...
  return (
    <div>
      ...
      <div
        ...
        style={{ display: OpenVideoModal ? 'block' : 'none' }}
      >
        <header>
          <h4>{videoDetail ? videoDetail.title : ''}</h4>
          <div className="close" onClick={() => setVideoModal(!OpenVideoModal)}>
            X
          </div>
        </header>
        <section>
          <div className="video-thumb">
            <img src={videoDetail.thumb} width="100%" height="200" />
          </div>
          <div className="video-name">
            <div className="video-title">{videoDetail.title}</div>
            <div className="video-subtitle">{videoDetail.subtitle}</div>
          </div>
      ...
      <div
        className="rightModal"
        style={{ display: CreateVideo ? 'block' : 'none' }}
      >
        <header>
          <h4>Create New Video</h4>
          <div className="close" onClick={() => setCreateVideo(!CreateVideo)}>
            X
          </div>
        </header>
        <section>
